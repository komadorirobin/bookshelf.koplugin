...
**Modification note:** `Inter-ExtraBold.ttf` is a copy of Inter's ExtraBold (800) static
instance with its name table rewritten to a standalone family ("Inter ExtraBold") so it
registers as its own family rather than collapsing under "Inter". Inter's OFL declares no
Reserved Font Name, so this renamed copy is compliant.
