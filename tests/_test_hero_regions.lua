...
test("ORDER lists all nine regions in render order", function()
    eq(#Regions.ORDER, 9)
    eq(Regions.ORDER[1], "status")
    eq(Regions.ORDER[2], "rating")
    eq(Regions.ORDER[3], "title")
    eq(Regions.ORDER[4], "subtitle")
    eq(Regions.ORDER[5], "author")
    eq(Regions.ORDER[6], "metadata")
    eq(Regions.ORDER[7], "description")
    eq(Regions.ORDER[8], "tags")
    eq(Regions.ORDER[9], "progress")
end)
...
test("read: returns defaults when nothing stored", function()
    ...
    assert(r.author.template:find("%translator", 1, true),
        "default author line should include the translator fallback token")
end)
