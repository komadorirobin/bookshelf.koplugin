...
local function authorsFromInfo(filepath, info)
    local authors = splitAuthors(info and info.authors, filepath)
...
local function _resolveTextMeta(filepath, cb, info, cp)
    ...
    out.series_name, out.series_num = seriesFromInfoOrEpub(
        filepath, out.series_name, out.series_num,
        _cpText(cp, "title")
    ...
    elseif cb and type(cb.authors) == "table" and #cb.authors > 0 then
        ...
        out.authors = authorsFromInfo(filepath, info)
    end
...
function Repo.buildBookMeta(filepath, opts)
    ...
    local book = {
        ...
        series      = (series_name and series_num
                        and (series_name .. " #" .. series_num))
                       or series_name,
...
local function _docSettingsPageCount(ds)
    ...
    if use_page_labels then
        local stable_pages = tonumber(_readDocSetting(ds, "pagemap_doc_pages"))
        if stable_pages then return stable_pages, "stable" end
    end

    local doc_pages = tonumber(_readDocSetting(ds, "doc_pages"))
    if doc_pages then return doc_pages, "render" end

    local rendered_pages = tonumber(_readDocSetting(ds, BOOKSHELF_RENDERED_PAGE_COUNT_KEY))
    if rendered_pages then return rendered_pages, "render" end
    ...
    local pages = type(stats) == "table" and tonumber(stats.pages)
    if pages then return pages, "render" end
    return nil
end
...
function Repo.buildBook(filepath, opts)
    ...
    local ds_page_count, ds_page_src = _docSettingsPageCount(ds)
...
function Repo.getReadingStatus(filepath)
    ...
    elseif status_name == "reading" or status_name == "on_hold"
            or status_name == "abandoned" or (pct and pct > 0) then
        return { state = "reading", pct = pct }
    end
    return nil
end
...
local function _getLightMetaCache(home, depth)
    ...
    elseif row_entry and row_entry.expires_at > now then
        ...
        if row_map then
            if not snapshot then fresh = true end
    ...
    local stale = row_map ~= nil and fresh == false
    local meta_map
    local count = 0
    local skipped = 0
    if row_map then
...
local function _resolveScopeAndOpts(scope_or_opts, maybe_opts)
    if type(scope_or_opts) == "table" and type(scope_or_opts.roots) == "table" then
        return scope_or_opts, maybe_opts
    end
    if type(scope_or_opts) == "table" then
        return nil, scope_or_opts
    end
    return nil, maybe_opts
end

local function _looksLikeFilter(t)
    if type(t) ~= "table" then return false end
    return t.statuses ~= nil or t.genres ~= nil or t.langs ~= nil
        or t.formats ~= nil or t.ratings ~= nil or t.collections ~= nil
        or t.folders ~= nil
end

local function _resolveScopeFilterOpts(sort_priority_override, scope_or_filter, maybe_filter, maybe_opts)
    local scope, filter, opts
    if type(sort_priority_override) == "table" and type(sort_priority_override.roots) == "table" then
        scope = sort_priority_override
        sort_priority_override = nil
        filter = scope_or_filter
        opts = maybe_filter
    elseif type(scope_or_filter) == "table" and type(scope_or_filter.roots) == "table" then
        scope = scope_or_filter
        filter = maybe_filter
        opts = maybe_opts
    elseif scope_or_filter == nil then
        if _looksLikeFilter(maybe_filter) then
            filter = maybe_filter
            opts = maybe_opts
        else
            opts = maybe_opts or maybe_filter
        end
    else
        filter = scope_or_filter
        opts = maybe_filter
    end
    return sort_priority_override, scope, filter, opts
end
...
function Repo.getAllFilepaths(scope)
    ...
    local paths = {}
    for _i, c in ipairs(candidatesForScope(home, depth, scope)) do
...
function Repo.getSeriesGroups(limit, offset, sort_priority_override, scope_or_filter, maybe_filter, maybe_opts)
    local scope, filter, opts
...
function Repo.getAuthors(limit, offset, sort_priority_override, scope_or_filter, maybe_filter, maybe_opts)
    local scope, filter, opts
    sort_priority_override, scope, filter, opts =
        _resolveScopeFilterOpts(sort_priority_override, scope_or_filter, maybe_filter, maybe_opts)
    local _t0 = _gettime()
    ...
    if not _hit then
        ...
        local list = _buildGroups("author", function(b) return b.authors end, true, scope)
        _authors_cache[key] = {
...
function Repo.getFolderSections(limit, offset, sort_priority_override, scope_or_filter, maybe_filter, maybe_opts)
    ...
    sort_priority_override, scope, filter, opts =
        _resolveScopeFilterOpts(sort_priority_override, scope_or_filter, maybe_filter, maybe_opts)
    local _t0 = _gettime()
    local lib_root = _normalizePath(_resolveLibraryRoot())
    ...
    local depth = BookshelfSettings.read("latest_walk_depth") or 3
    local scope_roots = _scopeRoots(scope)
    local roots = scope_roots or { root }
    local function within(path, parent)
        local prefix = _joinPath(parent, "")
        return path == parent or path:sub(1, #prefix) == prefix
    end
    -- A drilled/pinned folder narrows the profile, never replaces it. In
    -- particular, a parent folder must not mix manga into the library view.
    if opts and opts.root and scope_roots then
        roots = {}
        local seen = {}
        for _, scoped in ipairs(scope_roots) do
            local intersect = within(root, scoped) and root
                or (within(scoped, root) and scoped or nil)
            if intersect and not seen[intersect] then
                roots[#roots + 1] = intersect
                seen[intersect] = true
            end
        end
    end
    local FolderSections = require("lib/bookshelf_folder_sections")
    local sections, walks, light_caches, seen_files = {}, {}, {}, {}
    for i = 1, #roots do
        local section_root = roots[i]
        local walk_root = section_root
        -- Prefer a profile walk over a library-wide one so a manga-only
        -- view does not need a second scan of the prose library.
        for _, ancestor in ipairs(scope_roots or (lib_root and { lib_root } or {})) do
            if within(section_root, ancestor) then walk_root = ancestor; break end
        end
        local walk = cachedWalk(walk_root, depth)
        local inside = {}
        for _, c in ipairs(walk) do
            local fp = type(c) == "table" and c.fp or c
            if type(fp) == "string" and within(fp, section_root) and not seen_files[fp] then
                inside[#inside + 1] = c
                seen_files[fp] = true
            end
        end
        ...
        local grouped = FolderSections.group(walk, section_root)
        for j = 1, #grouped do
            local section = grouped[j]
            sections[#sections + 1] = section
            light_caches[section] = light_cache
        end
    end
    ...
    for si = 1, #sections do
        local s = sections[si]
        local light_cache = light_caches[s]
        local recs = {}
        for i = 1, #s.fps do
            local fp  = s.fps[i]
...
function Repo.getLanguages(limit, offset, sort_priority_override, scope_or_filter, maybe_filter, maybe_opts)
    local scope, filter, opts
    sort_priority_override, scope, filter, opts =
        _resolveScopeFilterOpts(sort_priority_override, scope_or_filter, maybe_filter, maybe_opts)
    local _t0 = _gettime()
...
function Repo.findGroup(kind, name, scope)
    ...
    if not cache then
        if     kind == "author"   then Repo.getAuthors(0, 0, nil, scope);      cache = _authors_cache[key]
        elseif kind == "series"   then Repo.getSeriesGroups(0, 0, nil, scope); cache = _series_cache[key]
        elseif kind == "genre"    then Repo.getGenres(0, 0, nil, scope);       cache = _genres_cache[key]
        ...
        elseif kind == "language" then Repo.getLanguages(0, 0, nil, scope);    cache = _languages_cache[key]
        end
...
function Repo.getNextUnreadInSeries(limit, offset, scope)
    ...
    for _, c in ipairs(cands) do
        local book = _lightMetaForFp(light_cache, c.fp)
        if book and book.series_name then
            local pct, status = Repo.readProgress(book.filepath)
    ...
    for _, sname in ipairs(order) do
        ...
        for i, b in ipairs(books) do
            ...
            local finished = b.status == "finished" or b.status == "complete" or pct >= 1
            local reading = not finished and (b.status == "reading" or b.status == "on_hold"
                or b.status == "abandoned" or (pct > 0 and pct < 1))
            if reading then
                started = true
                if not in_progress or (b.read_time or 0) > (in_progress.read_time or 0) then
                    in_progress = b
                end
            end
            if finished then
                ...
                latest_completed_time = math.max(latest_completed_time, b.read_time or 0)
            end
            if b.read_time and b.read_time > 0 then started = true end
        end

        local pick = in_progress
        if not pick and started and latest_completed_idx then
        ...
        if pick then
            picks[#picks + 1] = {
                ...
                latest_completed_time = latest_completed_time > 0 and latest_completed_time
                    or (pick.read_time or 0),
                series_name = sname,
            }
        end
    end
    ...
    logger.dbg(string.format("[bookshelf perf] getNextUnreadInSeries: %.0fms cands=%d picks=%d/%d",
        (_gettime() - _t0) * 1000, #cands, #out, total))
    return out, total
end
...
local function _bySourceCacheKey(source, filter, sort_priority, scope)
...
function Repo.getBySource(source, filter, sort_priority, offset, limit, scope_or_opts, maybe_opts)
    ...
    if (kind == "all" or kind == "folder") and Repo.spine_light then
        ...
        return Repo.getFolderSections(limit, offset, sort_priority, scope, filter, sopts)
    end
    if not has_status_filter then
        ...
        if not has_custom_sort then
            ...
            if kind == "latest"    then return Repo.getLatest(limit, offset, scope, opts) end
    ...
    if kind == "languages" then return Repo.getLanguages(limit, offset, sort_priority, scope, filter, opts) end
    ...
    local cache_key = _bySourceCacheKey(source, filter, sort_priority, scope)
